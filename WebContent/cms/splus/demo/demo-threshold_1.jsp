    ...
    <%    response.setCharacterEncoding("utf-8");
        ...
        if (!"".equals(msisdnIdCreditTmp) && !"".equals(currPoint)) {
            if (action != null && action.trim().equals("addCreditTmp")) {
                UserCreditTmpDAO userCreditTmpDAO = new UserCreditTmpDAO();
                int insertOrUpdate = 0;
                try {
                    insertOrUpdate = userCreditTmpDAO.insertOrUpdate(msisdnIdCreditTmp.trim(), Integer.valueOf(currPoint.trim()), Integer.valueOf(priPricePoint.trim()), Integer.valueOf(priPckPoint.trim()));
                } catch (Exception e) {
                    e.printStackTrace();
                }
                if (insertOrUpdate > 0) {
