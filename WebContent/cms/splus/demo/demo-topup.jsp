...
<%
    ...
    if(!"".equals(msisdnId) && !"".equals(type)) {
        CdrTopupQueue cdrTopupQueue = new CdrTopupQueue();
        boolean isInserted = false;
        try {
            cdrTopupQueue.setMsisdnId(msisdnId);
            cdrTopupQueue.setType(Integer.valueOf(type));
            cdrTopupQueue.setTimeMark(Long.valueOf(new SimpleDateFormat("yyyyMMddHHmmss").format(new Timestamp(System.currentTimeMillis()))));
            cdrTopupQueue.setFileName("WEB_PORTAL");

            CdrTopupQueueDAO cdrTopupQueueDAO = new CdrTopupQueueDAO();
            isInserted = cdrTopupQueueDAO.add2CdrTopup(cdrTopupQueue);
        } catch (Exception e) {
            e.printStackTrace();
        }
        if(isInserted) {
            resultInsertTopup = "<span style='font-size: 13px;margin: 10px 0 0 10px;color:green;'>Insert CDR_TOPUP_QUEUE success.</span>";
        }
        else {
            resultInsertTopup = "<span style='font-size: 13px;margin: 10px 0 0 10px;color:#dd3d08;'>Insert CDR_TOPUP_QUEUE unsuccess.</span>";
        }
    }
%>
<body>
