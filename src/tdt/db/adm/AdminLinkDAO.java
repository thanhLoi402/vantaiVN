...
public class AdminLinkDAO {
	...
	public boolean insertRow(AdminLink obj){
		...
		try{
			...
			if(!rs.next()){		
				rs.close();
				preStmt.close();
				strSQL= " insert into ADMIN_LINK (ID, NAME, URI, PARENT_ID, STATUS, POSITION, GEN_DATE, DISPLAY_TOP, IS_SELECT, IS_INSERT, IS_UPDATE, IS_DELETE) " +
						" values(ADMIN_LINK_SEQ.nextval, ?, ?, ?, ?, ?, sysdate, ?, ?, ?, ?, ?)";
				preStmt = conn.prepareStatement(strSQL);
	...
	public boolean updateRow(AdminLink obj){
		...
		try{
			conn = poolAdActive.getConnection();
			strSQL= " update ADMIN_LINK set NAME=?, URI=?, PARENT_ID=?, STATUS=?, POSITION=?, GEN_DATE=sysdate, DISPLAY_TOP=?, " +
	...
	public Vector<AdminLink> getTreeView() {		
		Connection conn = null;
		PreparedStatement preStmt = null;
		ResultSet rs = null;
		String strSQL = null;
		...
		try {
			conn = poolAdStandby.getConnection();
			strSQL = "select * from ADMIN_LINK where PARENT_ID = 0 and STATUS = 0 order by POSITION asc";				
			preStmt = conn.prepareStatement(strSQL);			
			rs = preStmt.executeQuery();
			result = new Vector<AdminLink>();			
			while (rs.next()) {
				id = rs.getBigDecimal("ID");		
				strSQL1 = "select * from ADMIN_LINK where STATUS = 0 connect by prior ID = PARENT_ID  start with ID in ?";
			...
			logger.error("getTreeView: Error executing SQL " + strSQL + ">>>" + e.toString());
			System.out.println("getTreeView: Error executing SQL " + strSQL + ">>>" + e.toString());
		} finally {	
			try {
				if(rs1!=null)
					rs1.close();
	...
	public Vector<AdminLink> getTreeView(int status) {		
		Connection conn = null;
		PreparedStatement preStmt = null;
		ResultSet rs = null;
		String strSQL = null;
		...
		try {
			conn = poolAdStandby.getConnection();
			strSQL = "select * from ADMIN_LINK where PARENT_ID = 0 "+where+" order by POSITION asc";				
			preStmt = conn.prepareStatement(strSQL);			
			rs = preStmt.executeQuery();
			result = new Vector<AdminLink>();			
			while (rs.next()) {
				id = rs.getBigDecimal("ID");		
				strSQL1 = "select * from ADMIN_LINK where ID>0 "+where+"  connect by prior ID = PARENT_ID  start with ID in ?";
				preStmt1 = conn.prepareStatement(strSQL1);
				preStmt1.setBigDecimal(1, id);
				rs1 = preStmt1.executeQuery();
				while(rs1.next()){
